import { MutantStatus, FileResultDictionary, MutantResult, FileResult } from 'mutation-testing-report-schema';
import { MetricsResult } from './api/MetricsResult';
import { flatMap, compareNames, normalizeFileNames } from './helpers';
...
export function calculateMetrics(files: FileResultDictionary): MetricsResult {
  ...
  return calculateDirectoryResults(normalizedFiles, ROOT_NAME);
}
...
function calculateDirectoryResults(files: FileResultDictionary, name: string): MetricsResult {
...
function calculateFileResult(fileName: string, file: FileResult): MetricsResult {
  return {
    file,
    name: fileName,
    childResults: [],
  ...
  return Object.keys(filesByDirectory)
    .map(directoryName => {
      if (filesByDirectory[directoryName].length > 1 || filesByDirectory[directoryName][0][0] !== directoryName) {
        ...
        return calculateDirectoryResults(directoryFiles, directoryName);
      } else {
        const fileName = filesByDirectory[directoryName][0][0];
        const file = filesByDirectory[directoryName][0][1];
        return calculateFileResult(fileName, file);
      }
    })
    .sort(compareNames);
}
